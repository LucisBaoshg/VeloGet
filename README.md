...
- **📦 轻量分发**: 安装包不再内置 `FFmpeg` / `yt-dlp`，首次启动会自动联网安装运行时组件。
...
```
**注意**:
1. Windows 安装包同样不再内置 `FFmpeg` / `yt-dlp`，首次启动会自动联网安装运行时组件。
2. 产物位于 `dist/`，包含：
...
2.  在“应用程序”中打开 **VeloGet**。
3.  首次启动时，应用会联网安装 `FFmpeg` / `yt-dlp` 运行时组件，请保持网络可用。
